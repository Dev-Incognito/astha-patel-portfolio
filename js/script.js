...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        // Animate capability bars if present
  ...
  const capBars = document.querySelectorAll('.cap-bar');
  ...
  capBars.forEach(bar => barObserver.observe(bar));
  ...
  const handleParallax = () => {
    if (window.__featureFlags && window.__featureFlags['parallax-bg'] === false) {
      // Reset transforms if disabled while scrolling
  ...
  const diffNumbers = document.querySelectorAll('.diff-number');
  ...
  diffNumbers.forEach(num => diffObserver.observe(num));
